...
#endif /* DBUG */
#else /* ifndef USE_LOCN_SERVER */
  if(chatcli_ctx->server_tcp_port == 0 || chatcli_ctx->server_udp_port == 0
...
void chat_interface(struct client_core* cli_core)
{
  ...
  while(cont)
  {
    ...
    {
      cont = handle_command_input(cli_core, buf);
    }
    else
    {
...
int main(int argc, char **argv)
{
  ...
  struct client_core* cli_core = create_client_core(chatcli_ctx.member_name,
      chatcli_ctx.server_host_name, chatcli_ctx.server_tcp_port, chatcli_ctx.server_udp_port);
