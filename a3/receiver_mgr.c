...
pid_t start_receiver(char* ctrl2rcvr_fname)
{
  ...
  if (receiver_pid == 0)
  {
    ...
    char *argv[] = {"xterm", "-e", "./receiver", "-f", ctrl2rcvr_fname, 0 };
...
struct receiver_manager* create_receiver_manager()
{
  ...
  struct receiver_manager* receiver_mgr = (struct receiver_manager*)malloc(sizeof(struct receiver_manager));
...
void receiver_printf(struct receiver_manager* receiver_manager, char* message)
{
  ...
  msgsnd(receiver_manager->ctrl2rcvr_qid, data, sizeof(msg_t) + sizeof(struct chat_msghdr) + msg->msg_len , 0);
  free(data);
}
