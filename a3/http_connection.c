...
struct http_connection* create_http_connection(const char* host_name, 
                                              int port,
                                              int* nerror){
    struct http_connection* http_con = (struct http_connection*)malloc(
      sizeof(struct http_connection)
    );
...
char* send_http_request(struct http_connection* http_con, char* url, u_int16_t* http_status, int* nerror){
  ...
  *http_status = 200;
  ...
  char* respond = (char*) malloc(strlen(previous));
  strcpy(respond, previous);
